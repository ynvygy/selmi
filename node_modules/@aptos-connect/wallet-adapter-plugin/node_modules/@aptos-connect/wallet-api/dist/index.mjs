...
import { Deserializer as Deserializer10, Serializer as Serializer10 } from "@aptos-labs/ts-sdk";
function serializeWalletResponse(args, serializeFn) {
  const serializer = new Serializer10();
  serializeFn(serializer, args);
  const data = serializer.toUint8Array();
  return { data };
}
function deserializeWalletResponse({ data }, deserializeFn) {
  const deserializer = new Deserializer10(data);
...
import { Deserializer as Deserializer15, Serializer as Serializer15 } from "@aptos-labs/ts-sdk";
...
function urlEncodeWalletRequest(request) {
  const serializer = new Serializer15();
...
function urlDecodeWalletRequest(encodedRequest) {
  const deserializer = new Deserializer15(base64ToBytes(encodedRequest));
  const name = deserializer.deserializeStr();
